...
from model.discrete import DiscreteModel
from model.helper import discretize_euler, discretize_rk4, discretize_exact
...
class ContinuousModel(Model):
    ...
    def linearize(self, x_op: npt.NDArray, u_op: npt.NDArray) -> 'ContinuousSystem':
        ...
        return ContinuousModel(dynamics_lin, self.dim_x, self.dim_u)
    ...
    def discretize(self,
        ...
        match discretization_method:
            case 'euler':
                def dynamics_discrete(x, u):
                    return discretize_euler(self.dynamics, x, u, sample_time)
                    #return x + sample_time * self.dynamics(x, u)
                
            case 'rk4':
                def dynamics_discrete(x, u):
                    return discretize_rk4(self.dynamics, x, u, sample_time)
        ...
        return DiscreteModel(dynamics_discrete, self.dim_x, self.dim_u, sample_time)


# TODO not really relevant for now
class LinearContinuousSystem(ContinuousModel):
    def __init__(self,
                 A: npt.NDArray,
                 B: npt.NDArray,
                 dim_x: int,
